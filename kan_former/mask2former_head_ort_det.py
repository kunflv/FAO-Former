...
class Mask2FormerHead_Ort_Det(MaskFormerHead):
    ...
    def __init__(self,
                 ...
                 feat_channels: int,
                 out_channels: int,
                 num_things_classes: int = 80,  # detection/ Instance segmentation / Semantic segmentation
                 num_stuff_classes: int = 53,  # 指不能区分的物体 主要用于全景分割(Panoramic segmentation)，也可以做Semantic segmentation
                 num_queries: int = 100,
    ...
    #                   attn_mask_target_size: Tuple[int, int], salience_map) -> Tuple[Tensor]:
        ...
        # # shape (batch_size, num_queries, cls)
        # cls_pred = cls_pred.view(b, q, -1)  # (4,21,22)
        # # shape (batch_size * num_queries, c)
        # mask_embed = self.mask_embed(kan_decoder_out)
        # # shape (batch_size, num_queries, c)
        ...
        attn_mask = attn_mask.flatten(2).unsqueeze(1).repeat(
            (1, self.num_heads, 1, 1)).flatten(0, 1)  # 根据注意力头数目（self.num_heads）来生成适应多头注意力机制的掩码
        attn_mask = attn_mask.sigmoid() < 0.5  # 注意力掩码二值化，将小于0.5的位置视为被忽略的位置，大于等于0.5的位置视为被关注的位置。
        attn_mask = attn_mask.detach()  # detach()函数将attn_mask张量从计算图中分离，使其成为不可训练的张量
    ...
    def forward(self, x: List[Tensor],
        ...
        for i in range(self.num_transformer_decoder_layers):
            ...
            attn_mask = attn_mask & mask_sum  # 执行按位与运算
