...
from mmseg.registry import MODELS   # 使用mmdet.registry不能在mmseg中注册模型
...
class MSDFDCPixelDecoder(BaseModule):
    ...
    def __init__(self,
        ...
        self.input_dat_conv = ModuleList()
        # channel_dim = [256, 512, 1024, 2048]  # setting for r50 & r101
        channel_dim = [192, 384, 768, 1536]  # setting for swin-l
        for i in range(self.num_input_levels):
            level_conv = Conv2d(channel_dim[i], out_channels, kernel_size=1,
                                stride=1, padding=0)
            self.input_dat_conv.append(level_conv)
    ...
    def forward(self, feats: List[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        for i in range(self.num_encoder_levels + 1):
            ...
            output_dat_feat = self.input_dats[level_idx](feat)
            out_conv_feat = self.input_dat_conv[level_idx](output_dat_feat)
            fdc_outs.append(out_conv_feat)

        """Feature fusion"""
        # gate_weights = [torch.sigmoid(fdc_out) for fdc_out in fdc_outs]
        # fused_outputs = [gate_weight * out for gate_weight, out in zip(gate_weights, outs)]
        fused_outputs = [fdc_out + out for fdc_out, out in zip(fdc_outs, outs)]

        multi_scale_features = fused_outputs[:self.num_outs]
