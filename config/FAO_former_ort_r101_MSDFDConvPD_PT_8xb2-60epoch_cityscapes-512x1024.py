...
custom_imports = dict(imports=[
    ...
    # 'kan_mask2former.custome_module.segmentors.k_encoder_decoder'
],
...
model = dict(
    ...
    decode_head=dict(
        ...
        num_queries=20,  # 20--->fg + bg
        num_transformer_feat_level=3,
        align_corners=False,
        pixel_decoder=dict(
            type='MSDFDCPixelDecoder',  # mmdet.MSDeformAttnPixelDecoder
            num_outs=3,
...
# train_dataloader = dict(sampler=dict(type='DefaultSampler', shuffle=True)  # 基于迭代次数的训练循环去掉此句
#                         )
...
param_scheduler = [
    ...
    dict(
        ...
        T_max=max_epochs,  # 半个周期的迭代次数，通常设为总epoch数
        eta_min=1e-6,  # 最小学习率
        by_epoch=True,  # 按epoch变化而不是按iteration
        begin=0,  # 从第0个epoch开始
        end=max_epochs  # 到第K个epoch结束
    )
]
...
default_hooks = dict(
    ...
    param_scheduler=dict(type='ParamSchedulerHook'),
    checkpoint=dict(
        type='CheckpointHook', by_epoch=True, interval=60,
        save_best='mIoU'),
    sampler_seed=dict(type='DistSamplerSeedHook'),
    visualization=dict(type='SegVisualizationHook'))
...
# """从给定路径将模型加载为预训练模型"""
load_from = r"kan_mask2former/checkpoints/cityscapes_r101_mIoU_79.83.pth"
